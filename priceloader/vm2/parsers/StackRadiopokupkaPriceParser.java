 ...
 */
public class StackRadiopokupkaPriceParser extends AbstractPriceLoaderVM2 {
    ...
    public String startWord = "Наименование";
    /*
     * карта выборки 
     */
    public int product_sku = 0;
    public int product_name = 1;
    public int product_manufacturer = 2;
    ...
    public void ParseExcelPrice(JProgressBar bar, String xml_id) {
        ...
        try {
            ...
            bar.setValue(0);
            Messages.show("Количество строк в листе№ "+sheet+" прайсе =" + s.getRows());
            Boolean start = false;
            ...
            for (int i = 0; i < s.getRows(); i++) {
                ...
                if (row.length < cell4getformat) {
                    Messages.show("Длина строки меньше чем келформат row.length="+row.length+" < cell4getformat="+cell4getformat);
                    continue;
                }
                
                if(endLine>0 && i>endLine){
                    Messages.show("Достигли конечной строки, заданной в настройках в настройках endLine="+endLine+" < i ="+i);
                    continue;
                }
                ...
                if (!start && 
                        ( row[cell4getformat].getContents().trim() == null && startWord == null
                        || row[cell4getformat].getContents().trim().equals(startWord) )
                        ) {
                    start = true;
                      Messages.show("Прайс  начался!");
                    continue;
                }
                ...
                if (!start) {
                      Messages.show("Прайс еще не начался |"+row[cell4getformat].getContents().trim()+"| !=|"+startWord+"|");
                    continue;
                }
                ...
                if (printDebug) {
                   ...
                   Messages.show("Обрабатывается лист № "+sheet+" строка:" + (i + 1)); 
                }
    ...
    public void farmPrice1Product(HashMap m) {
        ...
        try {
            ...
            if (cnt1 == 0) {
                ...
                p.put("slug", slug);
                p.put("metadesc", slug);
                p.put("metakey", p.get("product_name").toString());
                p.put("metadesc", p.get("product_name").toString());                
